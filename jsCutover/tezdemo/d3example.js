...
r_nodes = svg.append('g').attr('class','nodes r')
    .attr('transform', 'translate(' + (width + margin) +', 0)'),
g_nodes = svg.append('g').attr('class','nodes g')
    .attr('transform', 'translate(0, ' + (height + margin) + ')'),
b_nodes = svg.append('g').attr('class','nodes b')
...
// labels
rgb_nodes.append('text').text('rgb').attr('x', width / 2 )
    .attr('y', height + 20).style('text-anchor', 'middle');
r_nodes.append('text').text('r').attr('x', width / 2 )
    .attr('y', height + 20).style('text-anchor', 'middle');
g_nodes.append('text').text('g').attr('x', width / 2 )
    .attr('y', height + 20).style('text-anchor', 'middle');
b_nodes.append('text').text('b').attr('x', width / 2 )
    .attr('y', height + 20).style('text-anchor', 'middle');
...
function update(){
    rgb_nodes.selectAll('rect').transition()
        .style('fill', function(node){return rgb(node.value);});
    r_nodes.selectAll('rect').transition()
        .style('fill', function(node){return rgb([node.value[0], 0, 0]);});
    g_nodes.selectAll('rect').transition()
        .style('fill', function(node){return rgb([0, node.value[1], 0]);});
    b_nodes.selectAll('rect').transition()
        .style('fill', function(node){return rgb([0, 0, node.value[2]]);});
}
